...
// =====================================================================
//  GPSO COLLECTOR · Fiscalidad del Importador (cliente) · v2
//  app/recursos/fiscalidad/FiscalidadClient.jsx
//  Grafo de exploración progresiva: empiezas en un único nodo origen y
//  cada click hace brotar sus destinos — las conexiones se dibujan solas.
//  Nodos tipo orbe (estilo grafo Obsidian), sin cajas ni etiquetas de nivel.
// =====================================================================
...
// ---------------------------------------------------------------------
const NODES = {
  origen: { x: 150, y: 470, t: 'La operación', s: 'toca para abrir el mapa', origen: true,
  ...
  particular: { x: 480, y: 240, t: 'Particular', s: 'sin alta de actividad', revealBy: ['origen'],
  ...
  profesional: { x: 480, y: 470, t: 'Profesional', s: 'autónomo o empresa', revealBy: ['origen'],
  ...
  intermediario: { x: 480, y: 700, t: 'Intermediario', s: 'nunca toca la propiedad', revealBy: ['origen'],
    block: ['stock', 'rebu', 'rg', 'a45s', 'q3', 'b218', 'c300'],
  ...
  limite: { x: 790, y: 130, t: 'Límite del particular', s: 'la ocasionalidad', sat: true, revealBy: ['particular'],
    chips: ['Concepto'],
  ...
  stock: { x: 800, y: 355, t: 'Compra de stock', s: 'compras sin cliente esperando', revealBy: ['profesional'],
  ...
  carta: { x: 800, y: 590, t: 'Importación a la carta', s: 'el cliente ya existe', revealBy: ['particular', 'profesional', 'intermediario'],
  ...
  rebu: { x: 1120, y: 290, t: 'REBU', s: 'IVA solo sobre el margen', revealBy: ['stock', 'carta'],
...
<b>IVA a ingresar = margen − base</b></div><div class="hookline">El IVA del REBU sale de TU margen. Es la partida que casi nadie resta cuando calcula su ROI.</div>` },
  rg: { x: 1120, y: 510, t: 'Régimen General', s: 'IVA deducible · sobre el total', revealBy: ['stock', 'carta'],
...
Tu margen = base − coste total</div>` },
  serv: { x: 1120, y: 730, t: 'Servicios + suplido', s: 'el coche no es tuyo fiscalmente', revealBy: ['carta'],
  ...
  nifiva: { x: 1140, y: 105, t: 'NIF-IVA / ROI', s: 'capa · quién compra en la UE', sat: true, clip: true, revealBy: ['rg'],
    chips: ['Concepto', '🎬 clip'],
  ...
  ded: { x: 1140, y: 905, t: 'Deducción 50/100', s: 'capa · cuánto deduce el comprador', sat: true, revealBy: ['rg'],
    chips: ['Concepto'],
  ...
  a45s: { x: 1480, y: 175, t: 'Mercedes A45 S', s: 'stock · REBU', esCaso: true, clip: true, revealBy: ['rebu'],
...
<b>IVA a ingresar: 1.907,36 €</b></div><div class="kpi"><span class="k">Margen real neto</span><span class="v">3.066 €</span></div><div class="hookline">La hoja de cálculo marcaba un ROI del 10,15%. El real, tras restar el IVA REBU, es del 6,26%. Casi 4 puntos que se esfuman cuando llega el modelo 303.</div><div class="fotos">Fotos de la operación — pendiente de encajar</div>` },
  q3: { x: 1480, y: 345, t: 'Audi Q3', s: 'a la carta · REBU', esCaso: true, clip: true, revealBy: ['rebu'],
...
Margen REBU 5.651 € → <b>IVA: 981 €</b></div><div class="kpi"><span class="k">Beneficio neto previsto</span><span class="v">2.250 €</span></div><div class="hookline">20.900 € está POR DEBAJO del mínimo de mercado (21.500 €). Nadie pone ese precio a un stock — solo tiene sentido si ya sabes a quién se lo vendes.</div><div class="fotos">Fotos de la operación — pendiente de encajar</div>` },
  b218: { x: 1480, y: 515, t: 'BMW 218 GC', s: 'stock · Régimen General', esCaso: true, revealBy: ['rg'],
...
Total factura cliente: <b>24.990 €</b></div><div class="kpi"><span class="k">Margen (1.515,81 + 750 financiación)</span><span class="v">2.265 €</span></div><div class="hookline">Aquí el IVA no toca tu margen — lo paga el cliente aparte y tú solo lo pasas a Hacienda. Compara con el A45S: mismo negocio, dos matemáticas distintas.</div><div class="fotos">Fotos de la operación — pendiente de encajar</div>` },
  c300: { x: 1480, y: 685, t: 'Mercedes C300', s: 'a la carta · Régimen General', esCaso: true, clip: true, revealBy: ['rg'],
...
Factura: <b>36.500 €</b> = base 30.165,29 + IVA 6.334,71</div><div class="kpi"><span class="k">Margen real · ROI 10,2%</span><span class="v">2.793 €</span></div><div class="hookline">5.090 € de IVA extranjero parados en tesorería hasta que llega el reembolso. El coste oculto del que nadie habla al comprar en concesionario alemán.</div><div class="fotos">Fotos de la operación — pendiente de encajar</div>` },
  z4: { x: 1480, y: 855, t: 'BMW Z4', s: 'a la carta · servicios + suplido', esCaso: true, clip: true, revealBy: ['serv'],
...
const R = 11; // radio del orbe estándar

function edgePath([a, b]) {
  const na = NODES[a], nb = NODES[b];
  const left = na.x <= nb.x;
  const ra = (na.esCaso || na.origen) ? 15 : R;
  const rb = (nb.esCaso || nb.origen) ? 15 : R;
  const p1 = [na.x + (left ? ra + 3 : -(ra + 3)), na.y];
  const p2 = [nb.x + (left ? -(rb + 3) : rb + 3), nb.y];
...
export default function FiscalidadClient({ email, perfil }) {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 110; i++) {
      dots.push({
        x: Math.random() * 1700, y: Math.random() * 980,
    ...
    // centrar el viewport en el nodo origen al entrar
  ...
  const clickNodo = (id) => {
    // 1) expandir: hace brotar sus destinos
    if (!expandidos.has(id)) {
      const nx = new Set(expandidos); nx.add(id);
      setExpandidos(nx);
      // auto-scroll suave hacia lo que acaba de brotar
      if (viewRef.current) {
        const destino = Math.max(0, NODES[id].x - 340);
        viewRef.current.scrollTo({ left: destino, behavior: 'smooth' });
      }
    }
    // 2) iluminar el camino válido (solo entre lo visible)
    const s = new Set([id]);
    ...
    <div className="fisc-bg">
      ...
      <div className="viewport" ref={viewRef}>
        <div className={'canvas' + (lit ? ' dim' : '')}>
          ...
          <svg className="cosmos" viewBox="0 0 1700 980" aria-hidden="true">
          ...
          <svg className="wires" viewBox="0 0 1700 980">
            <defs>
              ...
              return (
                <g key={i}>
                  <path id={'w' + i} d={edgePath(e)} pathLength="1"
                    ...
                    filter={on ? 'url(#halo)' : undefined} />
                  {!e[2] && !reduceRef.current && (
                    <circle r="2.2" className={'spark' + (on ? ' on' : '')}>
      ...
      <style jsx>{`
        .fisc-bg{position:fixed;inset:0;background:radial-gradient(1200px 700px at 42% 45%, #0e1118 0%, #0a0c10 60%);color:#e9e6df;font-family:'Space Grotesk',sans-serif;font-weight:300;overflow:hidden}

        .fisc-top{position:absolute;top:0;left:0;right:0;z-index:40;display:flex;align-items:baseline;gap:18px;padding:20px 28px 14px;background:linear-gradient(to bottom,rgba(10,12,16,.94) 55%,rgba(10,12,16,0))}
        .marca{font-weight:600;font-size:15px;letter-spacing:.3px;color:#e9e6df}
        .marca .dot{color:#c9a14d}
        h1{font-family:'Cormorant Garamond',serif;font-weight:600;font-size:clamp(20px,3vw,30px);line-height:1;color:#ecdcae;margin:0}
        .badge-priv{font-size:10.5px;letter-spacing:.8px;color:#c9a14d;border:1px solid rgba(201,161,77,.5);border-radius:20px;padding:3px 10px}
        ...
        .canvas{position:relative;width:1700px;height:980px}
        ...
        .wires :global(.wire){fill:none;stroke:rgba(201,161,77,.30);stroke-width:1.1;stroke-dasharray:1;stroke-dashoffset:0;animation:traza .9s ease both;transition:stroke .35s,opacity .35s}
        ...
        .wires :global(.wire.on){stroke:#e3c987;stroke-width:1.5}
        .canvas.dim .wires :global(.wire:not(.on)){opacity:.14}
        .wires :global(.spark){fill:#c9a14d;opacity:.5;transition:opacity .35s}
        .wires :global(.spark.on){opacity:1;fill:#f0e2b6;r:2.8}
        .canvas.dim .wires :global(.spark:not(.on)){opacity:.05}
        @media (prefers-reduced-motion: reduce){.wires :global(.wire){animation:none}}

        /* ---------- nodos orbe ---------- */
        .nodo{position:absolute;transform:translate(-50%,-50%);display:flex;flex-direction:column;align-items:center;gap:9px;background:none;border:none;padding:8px;cursor:pointer;color:#e9e6df;font-family:'Space Grotesk',sans-serif;animation:brota .55s cubic-bezier(.2,.9,.3,1.4) both;transition:opacity .35s}
        ...
        .orbe{width:22px;height:22px;border-radius:50%;position:relative;background:radial-gradient(circle at 35% 30%, #f0e2b6 0%, #c9a14d 45%, #6b5526 100%);box-shadow:0 0 14px rgba(201,161,77,.45), 0 0 38px rgba(201,161,77,.15);transition:box-shadow .3s, transform .3s}
        .orbe.cerrado::after{content:'';position:absolute;inset:-7px;border-radius:50%;border:1px dashed rgba(201,161,77,.55);animation:girar 14s linear infinite}
        @keyframes girar{to{transform:rotate(360deg)}}
        @media (prefers-reduced-motion: reduce){.orbe.cerrado::after{animation:none}}

        .nodo:hover .orbe{box-shadow:0 0 22px rgba(201,161,77,.8), 0 0 60px rgba(201,161,77,.3);transform:scale(1.12)}
        .nodo.activo .orbe{box-shadow:0 0 26px rgba(240,226,182,.9), 0 0 70px rgba(201,161,77,.4)}

        .nodo.raiz .orbe{width:34px;height:34px;animation:latido 3.2s ease-in-out infinite}
        @keyframes latido{0%,100%{box-shadow:0 0 16px rgba(201,161,77,.5),0 0 44px rgba(201,161,77,.18)}50%{box-shadow:0 0 30px rgba(201,161,77,.9),0 0 80px rgba(201,161,77,.35)}}
        @media (prefers-reduced-motion: reduce){.nodo.raiz .orbe{animation:none}}

        .nodo.caso .orbe{width:28px;height:28px;background:radial-gradient(circle at 35% 30%, #fdf6e0 0%, #e3c987 40%, #8a6d2f 100%)}
        .nodo.sat .orbe{width:16px;height:16px;background:transparent;border:1.5px dashed rgba(201,161,77,.7);box-shadow:none}
        .nodo.sat:hover .orbe{box-shadow:0 0 14px rgba(201,161,77,.4)}

        .etq{text-align:center;max-width:170px}
        .etq .t{display:block;font-weight:500;font-size:13.5px;letter-spacing:.2px;text-shadow:0 2px 10px rgba(10,12,16,.9)}
        .nodo.caso .etq .t{color:#ecdcae}
        .etq .s{display:block;font-size:10.5px;color:#8b93a3;margin-top:2px;text-shadow:0 2px 8px rgba(10,12,16,.9)}
        .clipb{display:inline-block;font-size:9.5px;color:#0a0c10;background:#c9a14d;border-radius:4px;padding:1px 5px;margin-left:6px;font-weight:600;vertical-align:1px}
