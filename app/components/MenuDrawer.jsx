...
import { Menu, X, Home, Target, Gem, UploadCloud, User, ShieldCheck, LogOut, Moon, Sun } from 'lucide-react';
  ...
  return (
    <>
      ...
      <div className={`drawer ${open ? 'on' : ''}`}>
        ...
        <div className="dsep" />
        <a className="mitem" onClick={cambiarTema}>
          <span className="ico">{tema === 'dark' ? <Sun size={18} /> : <Moon size={18} />}</span>
          <div>{tema === 'dark' ? 'Tema claro' : 'Tema oscuro'}<small>Cambiar apariencia</small></div>
        </a>
